...
- `/elimina`: mostra a pagine i torrent completi e incompleti con la percentuale di avanzamento, consente di selezionarne uno solo e, dopo una seconda conferma esplicita, interrompe quel download e rimuove torrent e relativi dati dalla cartella download di Transmission.

Entrambi rileggono e convalidano i torrent al momento della conferma e non chiamano API di eliminazione di Radarr/Sonarr. `/clear` resta limitato ai torrent completati/in seed e conserva sempre i dati. `/elimina` include anche download incompleti, evidenzia percentuale e rischio nell'anteprima, e accetta soltanto percorsi contenuti in `TRANSMISSION_ALLOWED_DOWNLOAD_DIRS` (default `/downloads/complete`), interpretati nello spazio dei percorsi del container Transmission. Più directory possono essere fornite come lista separata da virgole. Il controllo viene ripetuto immediatamente prima della RPC di rimozione; Transmission non offre una verifica e cancellazione atomica del percorso, quindi l'operatore deve impedire spostamenti concorrenti dei torrent durante la conferma. Il container del bot continua a non avere mount delle cartelle download o media.
